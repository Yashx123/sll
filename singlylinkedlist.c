...
NODE create()
{
    ...
    scanf("%s%s%s%d%s",snode->usn, snode->name, snode->branch, &snode->sem,&snode->phone);
    snode->link=NULL;
    count++;
    return snode;
}
   ...
   if(start==NULL)
{
   ...
   while(cur!=NULL)
{
   printf("\n||%d|| USN:%s| Name:%s| Branch:%s| Sem:%d| Ph:%s|",num,cur->usn, cur->name,cur->branch,cur->sem,cur->phone);
   cur = cur->link;
   num++;
}
